...
function generateIssues(assets: Asset[]): FleetIssue[] {
  ...
  const lowBattery = assets
    .filter((a) => a.battery < 10 && a.status === "Active")
    .slice(0, 4);
  ...
  const speeding = assets
    .filter((a) => a.status === "Active" && (a.speed || 0) > 70)
    .slice(0, 3);
  ...
  gpsLoss.forEach((a, i) => {
    issues.push({
      ...
      driverName: a.driverName || "Unassigned",
      priority: "Medium",
      status: i === 0 ? "In Progress" : "Resolved",
      battery: a.battery,
      speed: 0,
      lat: a.lat,
      lng: a.lng,
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Real-time alerts from IoT modules — battery depletions, route
            deviations, signal loss, and emergency triggers.
          </p>
        </div>
        <Link
          to="/issues/new"
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-48 max-w-xs">
          ...
          <input
            type="text"
            placeholder="Search issues, assets, drivers..."
            value={search}
        ...
        <div className="flex items-center gap-1 bg-white border border-gray-200 rounded-xl p-1 shadow-sm">
          {["All", "Open", "In Progress", "Acknowledged", "Resolved"].map(
            (s) => (
              <button
                key={s}
            ...
            {issues.map((issue) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0 pr-6">
                    ...
                    <div className="flex items-center gap-4 mt-3 text-[11px] text-gray-400">
                      <span className="flex items-center gap-1">
                        <Battery size={11} />
                        {issue.battery.toFixed(0)}% battery
                      </span>
                      <span className="flex items-center gap-1">
                        <Zap size={11} />
                        {issue.speed} km/h
                  ...
                  {/* Category */}
                  <div
                    className={`inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-lg w-fit ${catColor}`}
                  >
                    <CatIcon size={10} />
                  ...
                  {/* Priority */}
                  <span
                    className={`text-[11px] font-semibold px-2 py-0.5 rounded-md w-fit ${priorityStyles[issue.priority]}`}
                  >
                    {issue.priority}
                  </span>
                  ...
                  {/* Status */}
                  <span
                    className={`text-[11px] font-semibold px-2 py-0.5 rounded-md w-fit ${statusStyles[issue.status]}`}
                  >
                    {issue.status}
                  </span>
