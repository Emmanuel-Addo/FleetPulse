...
import { Wrench, Clock, AlertCircle, CheckCircle2, Calendar, Plus } from 'lucide-react';
...
const Maintenance = () => {
  ...
  const maintenanceItems = useMemo(() =>
    assets
      ...
      .map(a => ({
        ...
        driver: a.driverName || '—',
        type: a.type,
        issue: a.battery < 15 ? 'Battery Critical' : a.battery < 30 ? 'Battery Low' : 'Scheduled Service',
  ...
  const highCount = maintenanceItems.filter(m => m.priority === 'High').length;
  const scheduled = maintenanceItems.filter(m => m.issue === 'Scheduled Service').length;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Track service schedules, issues, and fleet health.</p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          { label: 'High Priority', value: highCount, icon: AlertCircle, color: 'text-red-500' },
          { label: 'In Progress', value: inProgress, icon: Wrench, color: 'text-amber-600' },
      ...
      <Card>
        ...
        <CardContent className="p-0">
          <div className="grid grid-cols-6 px-5 py-2.5 border-y border-border bg-secondary text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
            <span className="col-span-2">Vehicle</span><span>Issue</span><span>Battery</span><span>Priority</span><span>Action</span>
          </div>
          <div className="divide-y divide-border max-h-[500px] overflow-y-auto">
            {maintenanceItems.slice(0, 100).map(item => (
              <div key={item.id} className="grid grid-cols-6 px-5 py-4 items-center hover:bg-accent/50 transition-colors">
                <div className="col-span-2">
                  <p className="text-sm font-semibold text-gray-800">{item.vehicle}</p>
                  <p className="text-xs text-muted-foreground">{item.type} · {item.driver}</p>
                </div>
                <p className="text-sm text-gray-600">{item.issue}</p>
                <div className="flex items-center gap-2">
                  <Progress value={item.battery} className="w-16 h-1" />
                  <span className="text-xs text-gray-500">{item.battery.toFixed(0)}%</span>
                </div>
                <Badge variant={priorityStyle[item.priority] as any} className="w-fit">
                  {item.priority}
                </Badge>
