...
function AlertBadge({ type }: { type: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-md border ${c.cls}`}
    >
      {c.icon} {type}
    </span>
  );
    ...
    >
      ...
      <button onClick={onClick} className="w-full text-left px-4 py-3.5">
        <div className="flex items-start gap-3">
          <div
            className={`mt-0.5 w-8 h-8 rounded-xl flex items-center justify-center shrink-0 ${isSelected ? "bg-white/10" : "bg-gray-100"}`}
          >
            <Icon
              size={16}
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-1 shrink-0">
                <span
                  className={`w-1.5 h-1.5 rounded-full ${statusDot[asset.status]}`}
                />
                <span
                  className={`text-[11px] font-medium ${isSelected ? "text-gray-300" : statusText[asset.status]}`}
                >
                  {asset.status}
                </span>
              </div>
            ...
            {asset.driverName && (
              <div
                className={`flex items-center gap-1 text-[11px] mt-0.5 ${isSelected ? "text-gray-400" : "text-gray-500"}`}
              >
            ...
            <div
              className={`flex items-center gap-3 mt-2 text-[11px] ${isSelected ? "text-gray-300" : "text-gray-500"}`}
            >
              <span className="flex items-center gap-1">
                <Battery
                  size={11}
                  className={asset.battery < 15 ? "text-red-400" : ""}
                />
                <span
                  className={
                    asset.battery < 15 ? "text-red-400 font-semibold" : ""
                  }
                >
              ...
              <span className="flex items-center gap-1">
                <MapPin size={11} /> {asset.lat.toFixed(2)},{" "}
                {asset.lng.toFixed(2)}
              </span>
            </div>
      ...
      <div
        className={`px-4 pb-3 flex items-center gap-1.5 ${isSelected ? "flex" : "opacity-0 hover:opacity-100 transition-opacity group-hover:opacity-100 h-0 overflow-hidden group-hover:h-auto"}`}
      >
        {isSelected && (
          <>
            <span
              className={`text-[10px] font-medium mr-1 ${isSelected ? "text-gray-400" : "text-gray-400"}`}
            >
              Actions:
            </span>
            <button
...
function VehicleDetail({
  ...
  return (
    <div className="absolute bottom-6 right-6 z-[600] w-80 bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden">
      <div
        className={`px-4 py-3 flex items-center justify-between ${asset.status === "Active" ? "bg-emerald-600" : asset.status === "Offline" ? "bg-red-600" : "bg-amber-500"}`}
      >
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2.5">
          {[
            {
              label: "Battery",
              value: `${asset.battery.toFixed(0)}%`,
              icon: Battery,
              warn: asset.battery < 15,
            },
            {
              label: "Speed",
              value: `${asset.speed || 0} km/h`,
              icon: Zap,
              warn: (asset.speed || 0) > 70,
            },
            ...
            <div
              key={label}
              className={`rounded-xl p-3 ${warn ? "bg-red-50 border border-red-100" : "bg-gray-50 border border-gray-100"}`}
            >
              ...
              <p
                className={`text-sm font-bold ${warn ? "text-red-600" : "text-gray-900"}`}
              >
                {value}
              </p>
            </div>
      ...
      >
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-bold text-red-600">
            {alertAssets.length} Active Alert{alertAssets.length > 1 ? "s" : ""}
          </span>
        </div>
        {open ? (
...
const LiveTracking = () => {
  ...
  const listAssets: FleetAsset[] = useMemo(() => {
    ...
    if (activeTab === "Alerts")
      list = list.filter((a) => getAlertType(a) !== null);
  ...
  const handleSelectAsset = useCallback((asset: { id: string; name: string; type: string; status: string; battery: number; lat: number; lng: number; speed?: number; driverName?: string; tags?: string[]; lastUpdated: number }) => {
  ...
  const handleAction = useCallback(
    async (id: string, action: string) => {
      const updates: Partial<Asset> =
        action === "Lock" ? { status: "Offline" } : { status: "Maintenance" };
      try {
        await performRemoteAction(id, action as 'Lock' | 'Reroute' | 'Maintenance', updates);
  ...
  const offlineCount = useMemo(
    () => allAssets.filter((a) => a.status === "Offline").length,
    [allAssets],
  );
  const alertCount = useMemo(
    () => allAssets.filter((a) => a.battery < 10 || (a.speed || 0) > 70).length,
    [allAssets],
  );
      ...
      <div className="w-[420px] flex flex-col shrink-0 border-l border-gray-100 bg-white z-10 shadow-sm mr-4">
        ...
        <div className="grid grid-cols-3 border-b border-gray-100">
          {[
            ...
            <div
              key={label}
              className="text-center py-3 border-r border-gray-100 last:border-r-0"
            >
        ...
        <div className="px-3 py-2 border-b border-gray-100 flex items-center gap-2">
          ...
          />
          <span className="text-[10px] font-bold text-gray-700 w-7 text-right">
            {batteryMin}%
          </span>
          {batteryMin > 0 && (
            <button
              onClick={() => setBatteryMin(0)}
