...
import {
  ArrowLeft, Save, X, Battery, MapPin, Wifi, Zap,
  ShieldAlert, AlertTriangle, ChevronDown, User, Clock,
  Navigation, Radio, AlertOctagon
} from 'lucide-react';
import { toast } from 'react-toastify';

const ISSUE_CATEGORIES = [
  { label: 'Battery Depletion',     icon: Battery,       desc: 'Battery critically low during transit' },
  { label: 'Route Deviation',       icon: Navigation,    desc: 'Asset deviated from assigned route' },
  { label: 'GPS / Signal Loss',     icon: Wifi,          desc: 'IoT module GPS unresponsive' },
  { label: 'Emergency SOS',         icon: AlertOctagon,  desc: 'Driver-triggered emergency alert' },
  { label: 'Engine Fault',          icon: AlertTriangle, desc: 'Engine fault code via OBD module' },
  { label: 'Speeding Violation',    icon: Zap,           desc: 'Asset exceeded zone speed limit' },
  { label: 'Communication Failure', icon: Radio,         desc: 'Telemetry stream lost or degraded' },
  { label: 'Unauthorized Stop',     icon: MapPin,        desc: 'Vehicle stopped outside approved zones' },
] as const;
...
const ReportIssue = () => {
  ...
  const handleAssetChange = (assetId: string) => {
    ...
    if (!form.category && asset) {
      ...
      else if (asset.status === 'Offline') set('category', 'Communication Failure');
  ...
  const handleCategoryChange = (cat: Category) => {
    ...
    const summaries: Record<string, string> = {
      'Battery Depletion':     asset ? `${asset.name} battery at ${asset.battery.toFixed(0)}% — critically low during active transit` : '',
      'Route Deviation':       asset ? `${asset.name} deviated from assigned dispatch route by ${(Math.random() * 3 + 0.5).toFixed(1)} km` : '',
      'GPS / Signal Loss':     asset ? `${asset.name} GPS module unresponsive — last fix: ${asset.lat.toFixed(4)}, ${asset.lng.toFixed(4)}` : '',
      'Emergency SOS':         asset ? `Emergency SOS triggered by ${asset.driverName || 'driver'} in ${asset.name}` : '',
      'Engine Fault':          asset ? `Engine fault code detected in ${asset.name} via IoT OBD module` : '',
      'Speeding Violation':    asset ? `${asset.name} recorded at ${asset.speed} km/h in a restricted zone` : '',
      'Communication Failure': asset ? `Telemetry stream lost for ${asset.name} — no data received for ${Math.floor(Math.random() * 20 + 5)} minutes` : '',
      'Unauthorized Stop':     asset ? `${asset.name} stopped outside approved delivery zone` : '',
    };
    setForm(prev => ({
      ...prev,
      category: cat,
  ...
  const handleSave = () => {
    if (!validate()) return;
    if (form.isEmergency) {
      toast.error(`🚨 Emergency alert dispatched for ${selectedAsset?.name || 'vehicle'}!`);
    } else {
      toast.success(`Issue reported successfully for ${selectedAsset?.name || 'vehicle'}`);
    }
    navigate('/issues');
  };
    ...
    <div className="max-w-3xl mx-auto space-y-6 pb-12">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">Report Telemetry Issue</h1>
            <p className="text-sm text-gray-500 mt-0.5">Log a fleet alert for dispatch tracking and resolution</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleSave}
            ...
            <Save size={14} /> {form.isEmergency ? 'Dispatch Emergency' : 'Save Issue'}
          </button>
        </div>
      </div>
      ...
      {form.isEmergency && (
        <div className="flex items-center gap-3 bg-red-600 text-white rounded-2xl px-5 py-4">
          ...
          <div>
            ...
            <p className="text-sm text-red-100 mt-0.5">This issue will be immediately escalated to all dispatchers and flagged as critical.</p>
          </div>
        </div>
      )}

      {/* 1. Issue Category */}
      <SectionCard title="Issue Category" desc="Select the type of telemetry alert or fleet incident">
        <div className="grid grid-cols-2 gap-3">
          {ISSUE_CATEGORIES.map(({ label, icon: Icon, desc }) => (
            ...
            >
              <Icon size={18} className={`shrink-0 mt-0.5 ${form.category === label ? 'text-white' : 'text-gray-500'}`} />
              <div>
                <p className={`text-sm font-bold ${form.category === label ? 'text-white' : 'text-gray-900'}`}>{label}</p>
                <p className={`text-xs mt-0.5 ${form.category === label ? 'text-gray-300' : 'text-gray-500'}`}>{desc}</p>
              </div>
            </button>
          ))}
        </div>
        {errors.category && <p className="text-sm text-red-500 mt-3">{errors.category}</p>}
      </SectionCard>
        ...
        <div className="space-y-5">
          <div className="grid grid-cols-2 gap-5">
            <div>
              ...
              <div className="relative">
                <select
                  ...
                  className={`w-full appearance-none border rounded-xl px-4 py-3 text-sm text-gray-900 pr-9 bg-white focus:outline-none focus:ring-2 focus:ring-gray-300 transition ${
                    errors.assetId ? 'border-red-400 bg-red-50/20' : 'border-gray-300'
                  }`}
                >
              ...
              {errors.assetId && <p className="text-sm text-red-500 mt-1.5">{errors.assetId}</p>}
            </div>
          ...
          {selectedAsset && (
            <div className="rounded-xl bg-gray-50 border border-gray-200 p-5">
              ...
              <div className="grid grid-cols-4 gap-3">
                {[
                  { icon: Battery,    label: 'Battery',   value: `${selectedAsset.battery.toFixed(0)}%`,       warn: selectedAsset.battery < 15 },
                  { icon: Zap,        label: 'Speed',     value: `${selectedAsset.speed || 0} km/h`,           warn: (selectedAsset.speed || 0) > 70 },
                  { icon: MapPin,     label: 'Latitude',  value: selectedAsset.lat.toFixed(5),                 warn: false },
                  { icon: MapPin,     label: 'Longitude', value: selectedAsset.lng.toFixed(5),                 warn: false },
                  ...
                  <div key={label} className={`rounded-lg p-3 border ${warn ? 'bg-red-50 border-red-200' : 'bg-white border-gray-200'}`}>
                    <div className="flex items-center gap-1.5 mb-1">
                      <Icon size={12} className={warn ? 'text-red-500' : 'text-gray-400'} />
                      <span className="text-xs text-gray-500">{label}</span>
                    </div>
                    <p className={`text-base font-bold ${warn ? 'text-red-600' : 'text-gray-900'}`}>{value}</p>
                  </div>
                ))}
              </div>
              <div className="flex items-center gap-3 mt-4 pt-3 border-t border-gray-200">
                <span className={`text-xs font-bold px-2.5 py-1 rounded-lg ${
                  ...
                  selectedAsset.status === 'Offline' ? 'bg-red-100 text-red-700' :
                  'bg-amber-100 text-amber-700'
                }`}>{selectedAsset.status}</span>
                {selectedAsset.driverName && (
                  <span className="text-sm text-gray-600 flex items-center gap-1.5">
                    <User size={13} className="text-gray-400" /> {selectedAsset.driverName}
                  </span>
                )}
        ...
        <div className="space-y-5">
          <div>
            ...
            <input
              ...
              className={`w-full border rounded-xl px-4 py-3 text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-300 transition ${
                errors.summary ? 'border-red-400 bg-red-50/20' : 'border-gray-300 bg-white'
              }`}
            />
            {errors.summary && <p className="text-sm text-red-500 mt-1.5">{errors.summary}</p>}
          </div>
          ...
          {form.category === 'Route Deviation' && (
            <div>
              ...
              <input type="number" step="0.1" placeholder="e.g. 2.4"
                value={form.deviationDistance} onChange={e => set('deviationDistance', e.target.value)}
              ...
              />
              <p className="text-xs text-gray-500 mt-1.5">Distance in km the vehicle has strayed from the assigned route path</p>
            </div>
          )}
          ...
          {form.category === 'Engine Fault' && (
            <div>
              ...
              <input type="text" placeholder="e.g. P0401, P0300"
                value={form.faultCode} onChange={e => set('faultCode', e.target.value)}
              ...
              />
              <p className="text-xs text-gray-500 mt-1.5">OBD-II fault code reported by the vehicle's IoT telemetry module</p>
            </div>
          )}
          ...
          {form.category === 'Speeding Violation' && (
            <div>
              ...
              <input type="number" placeholder="e.g. 87"
                value={form.speedRecorded} onChange={e => set('speedRecorded', e.target.value)}
                className="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-gray-300 bg-white"
              />
            </div>
          )}

          {(form.category === 'GPS / Signal Loss' || form.category === 'Communication Failure') && (
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label>Last Known Latitude</Label>
                <input type="text" placeholder="e.g. 41.8781"
                  value={form.lastKnownLat} onChange={e => set('lastKnownLat', e.target.value)}
                  className="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm text-gray-900 font-mono focus:outline-none focus:ring-2 focus:ring-gray-300 bg-white"
                />
              </div>
              <div>
                <Label>Last Known Longitude</Label>
                <input type="text" placeholder="e.g. -87.6298"
                  value={form.lastKnownLng} onChange={e => set('lastKnownLng', e.target.value)}
                  className="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm text-gray-900 font-mono focus:outline-none focus:ring-2 focus:ring-gray-300 bg-white"
                />
              </div>
            </div>
      ...
      {/* 4. Alert Flags */}
      <SectionCard title="Alert Flags" desc="Configure how this issue should be handled by the dispatch system">
        <div className="space-y-4">
          {[
            { key: 'isEmergency',      label: 'Mark as Emergency',          desc: 'Immediately escalate to all dispatchers and trigger priority queue' },
            { key: 'dispatchRequired', label: 'Dispatch Required',           desc: 'A dispatch unit must be sent to the vehicle location' },
          ].map(({ key, label, desc }) => (
            <div key={key} className="flex items-start justify-between p-4 rounded-xl border border-gray-200 bg-gray-50/50">
              <div>
      ...
      <div className="flex justify-end gap-3 pt-2">
        ...
        <button onClick={handleSave}
          className={`text-sm font-bold px-8 py-3 rounded-xl transition shadow-sm ${
            form.isEmergency ? 'bg-red-600 hover:bg-red-700 text-white' : 'bg-gray-900 hover:bg-gray-700 text-white'
          }`}>
