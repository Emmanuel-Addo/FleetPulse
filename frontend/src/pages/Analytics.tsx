...
const Analytics = () => {
  ...
  const stats = useMemo(() => {
    ...
    const avgSpeed =
      assets
        ...
        .reduce((s, a) => s + (a.speed || 0), 0) / (active || 1);
  ...
  const statuses = [
    ...
    {
      label: "Idle",
      count: assets.filter((a) => a.status === "Idle").length,
      color: "bg-gray-300",
    },
    {
      label: "Maintenance",
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">
          Fleet performance insights and operational trends.
        </p>
      </div>
            ...
            <Card key={label}>
              <CardContent className="p-5">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span
                    className={`text-xs font-semibold flex items-center gap-0.5 ${up ? "text-emerald-600" : "text-red-500"}`}
                  >
      ...
      <div className="grid grid-cols-3 gap-5">
        ...
        <Card className="col-span-2">
          <CardHeader>
            ...
            <CardDescription>
              Active vehicle percentage per day (last 7 days)
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex items-end gap-3 h-40">
              {days.map((day, i) => (
                ...
                >
                  ...
                  <div
                    className="w-full rounded-t-md bg-gray-100 relative"
                    style={{ height: "100px" }}
                  >
                    <div
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Current distribution across all assets
            </CardDescription>
          </CardHeader>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Distribution of battery levels across the entire fleet
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex gap-2 flex-wrap">
            {[
              {
                label: "0–20%",
                count: assets.filter((a) => a.battery <= 20).length,
                color: "bg-red-500",
                text: "text-white",
              },
              {
                label: "21–40%",
                count: assets.filter((a) => a.battery > 20 && a.battery <= 40)
                  .length,
                color: "bg-amber-400",
                text: "text-white",
              },
              {
                label: "41–60%",
                count: assets.filter((a) => a.battery > 40 && a.battery <= 60)
                  .length,
                color: "bg-yellow-300",
                text: "text-gray-800",
              },
              {
                label: "61–80%",
                count: assets.filter((a) => a.battery > 60 && a.battery <= 80)
                  .length,
              ...
              {
                label: "81–100%",
                count: assets.filter((a) => a.battery > 80).length,
                color: "bg-emerald-500",
                text: "text-white",
              },
              ...
              <div
                key={label}
                className={`flex-1 min-w-28 ${color} rounded-xl p-4 flex flex-col items-center`}
              >
                <p className={`text-2xl font-bold ${text}`}>{count}</p>
                <p className={`text-xs mt-1 font-medium ${text} opacity-80`}>
                  {label}
                </p>
              </div>
            ))}
