...
const Drivers = () => {
  ...
  const drivers = useMemo(() => {
    ...
    return assets
      .filter(
        (a) =>
          a.driverName && !seen.has(a.driverName) && seen.add(a.driverName!),
      )
      .map((a) => ({
        name: a.driverName!,
        vehicle: a.name,
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          ...
          {
            label: "Avg. Rating",
            value: `${(drivers.reduce((s, d) => s + parseFloat(d.rating), 0) / (drivers.length || 1)).toFixed(1)} ★`,
            icon: Star,
        ...
        <CardContent className="p-0">
          ...
          <div className="divide-y divide-border max-h-[480px] overflow-y-auto">
            {filtered.slice(0, 100).map((driver) => (
              <div
                key={driver.name}
                className="grid grid-cols-6 px-5 py-3.5 items-center hover:bg-accent/50 transition-colors cursor-pointer"
              >
