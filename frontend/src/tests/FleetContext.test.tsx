...
const TestComponent = ({ onBattery }: { onBattery?: (v: number) => void } = {}) => {
  ...
  return (
    <div>
      ...
      <button data-testid="action-btn" onClick={() =>
        performRemoteAction('1', 'Lock', { status: 'Offline' }).catch(() => {})
      }>Lock</button>
      <button data-testid="success-btn" onClick={() =>
        performRemoteAction('1', 'Maintenance', { status: 'Maintenance' }).catch(() => {})
      }>Maintain</button>
      <button data-testid="set-offline" onClick={() =>
        dispatch({ type: 'SET_OFFLINE', payload: true })
      }>Go Offline</button>
      <button data-testid="set-online" onClick={() =>
        dispatch({ type: 'SET_OFFLINE', payload: false })
      }>Go Online</button>
      <button data-testid="update-battery" onClick={() =>
        dispatch({ type: 'UPSERT_ASSETS', payload: [{ ...mockAsset, id: '1', battery: 42 }] })
...
describe('FleetContext', () => {
  ...
  it('should optimistically update and then rollback on API failure', async () => {
    ...
    const { getByTestId } = render(
      <FleetProvider><TestComponent /></FleetProvider>
    );

    expect(getByTestId('status').textContent).toBe('Active');
    act(() => { getByTestId('action-btn').click(); });
  ...
  it('should apply optimistic update that persists on success', async () => {
    ...
    act(() => { getByTestId('success-btn').click(); });
  ...
  it('should set offline state', () => {
    ...
    act(() => { getByTestId('set-offline').click(); });
    expect(getByTestId('offline').textContent).toBe('true');

    act(() => { getByTestId('set-online').click(); });
    expect(getByTestId('offline').textContent).toBe('false');
  });

  it('should persist assets to localStorage on upsert', () => {
    render(<FleetProvider><TestComponent /></FleetProvider>);
  ...
  it('should throw when useFleet is used outside FleetProvider', () => {
    ...
    expect(() => render(<BadComponent />)).toThrow('useFleet must be used within a FleetProvider');
    spy.mockRestore();
  });
  ...
  it('should update asset via UPSERT_ASSETS with merged fields', () => {
    ...
    const { getByTestId } = render(
      <FleetProvider>
        <TestComponent onBattery={(v) => { capturedBattery = v; }} />
      </FleetProvider>
    );

    expect(getByTestId('battery').textContent).toBe('100');

    act(() => { getByTestId('update-battery').click(); });
